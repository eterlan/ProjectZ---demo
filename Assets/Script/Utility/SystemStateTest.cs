...
{
    public int Count;
    ComponentGroup m_CreateGroup;
    ComponentGroup m_DestroyGroup;
    protected override void OnUpdate()
    {
        for (int i = 0; i < m_CreateGroup.CalculateLength(); i++)
        {
            ...
            EntityManager.DestroyEntity(GetComponentGroup(new[] { ComponentType.ReadWrite<Normal>(), typeof(State)}));
        }
    ...
    {
        m_CreateGroup = GetComponentGroup( new EntityArchetypeQuery {
            All = new[] {
        ...
        m_DestroyGroup = GetComponentGroup( new EntityArchetypeQuery {
            All = new[] {
                ComponentType.ReadWrite<State>(),
            },
            None = new[] {
